...
​	그리고 예제를 보면 라인끝에 세미콜론 `;` 를  일부러 넣지 않은 것을 발견할 수 있다. 붙히게 되면 구문이 되기 때문에 표현식 자체로 의미 부여를 위해 붙히지 않았다. 당연히 실행에는 아무런 문제가 없는 온전한 JavaScript 코드이다. 방금 "실행"이라 했는데,  JavaScript의 실행 단위는 **구문(또는 문장, Statement)**이다. 그러면 표현식으로만 되어 있는 예제 코드는 엔진에서 실행은 되는 것인가? 실행된다면 어디부터 어디까지 어떤 단위로 실행 될까? 
...
구문과 표현식의 구분에 대한 이해는 해당 주제와 관련해서 이해해 나가야 한다. 변수, 연산자, 함수, 객체, 스코프 이런 식으로 하나의 주제에 맞게 해당 주제의 코드에서 구문과 표현식을 구분해 보고 특징을 찾는 방법이 좋은 것 같다. 여기서는 변수와 관련해서 기초적인 구문과 표현식의 특징을 살펴보자. 

#### 4.1 구문

처음 변수 설명을 시작하면서 "프로그램은 지속적으로 어떤 목적을 가지고 데이터를 특정 알고리즘에 의해 변화 시키는 과정이라 볼 수 있다" 고 프로그램에 대한 정의를 내렸다. 프로그램은 이 "특정 알고리즘에 의해 변화 시키는 과정" 들의 모음으로 볼 수 있다. 이 "특정 알고리즘에 의해 변화 시키는 과정"을  **명령(Instruction)** 또는 **구문(문장, Statement)** 이라 한다. 따라서 구문이 실행되는 것이고 실행 단위가 된다.
...
​	프로그래밍을 입문하게 되면 초입에 배우게 되는 가장 기본적인 구문들만 나열 하였다. 이외에도 함수정의, 블록({ }) 구문과 같이 다소 혼란스럽게 하는 구문들은 제외하였고 뒤에서 다루겠다.

#### 4.2 표현식의 실행	

​	JavaScript의 실행 단위는 구문이다. 엔진의 파서가 구문의 처음과 끝을 발견하면 즉시 그 구문을 실행한다. 표현식은 원칙적으로 실행 대상이 아니라 구문의 구성 요소라고 위에서 말했다. 구문에 포함되어 있는 표현식이 구문아 실행될   표현식이 표현하는 값으로 평가(evalue)될 뿐이다.

​	어디까지나 원칙이 그렇다는 것이고 사실 실행되기도 한다. 하지만 구문 단위 실행의 원칙을 깨지 않는다. 즉, 표현식이 실행되는 것이 아니고 **표현식 구문(Expression Statement)** 이 실행되는 것으로 이해해야 한다. 표현식 뒤에 `;` 를 붙히면 표현식 구문이 되어 실행된다. 앞의 '3.1 리터럴과 식별자의 구분 : ex01.js' 예제의 세 개의 각각의 라인은 모두 대입 연산자 표현식이며 뒤에 `;`를 붙혀 표현식 구문으로 실행된다. 실행은 라인의 구문에 포함된 대입 연산자 표현식이 평가되는 것이 전부이다.
...
​	 JavaScript 엔진은 공백을 기본적으로 무시하기 때문에  `,`를 사용하지 않으면 엔진은 3번 라인을 `1=10j=20;` 으로 인식해서 Syntax Error를 발생할 것이다. 따라서 `,`  로 두 표현식을 구분하는 역할을 한다.  그러면 다음 예제로 1번과 2번 라인을 구분하고 있는 개행의 역할을 생각해 보자.
...
​	이렇게 코딩할 일은 없지만 문법과 실행에는 문제가 없는 코드다. 엔진은 개행을 무시하지 않는다. 공백처럼 무시했다면 다음 코드는 실행이 잘 될 것이다. 하지만 문법 오류가 발생하기 때문에 공백처럼 무시하지 않는다는 사실을 알 수 있다.
...
엔진은 개행을 토큰 구분으로 사용한다. 사실 표현식을 포함하여 모둔 구문들은 **토큰(Token)**으로 더 쪼개져서 해석되고 실행되는 데 i=10 표현식은 `i`, `=`, `10` 으로 분리되어 해석된다. 위 예제는 엔진이 쪼갤 필요없이 개행으로 구분하여 세 개의 토큰으로 받아 질 것이다.

​	` i = 10, j = 20` 표현식 구문의 실행은 `i` , `=`,  `10` ,` j`, `=` , `20` 6개의 토큰으로 쪼개져서 왼쪽부터 해서되다가 10에서 하나의 표현식 평가가 완료되고 다시 20에서 하나의 표현식 평가가 될 것이다.  `,`  는 하나의 라인에서 여러 표현식을 분리하는 것 처럼 보이지만 사실은 완결된 표현식들을 연결하는 역할을 한다. 따라서 다음 코드도 문법에 맞다.
...
`console`, `.` , ` log('Hello World')` 로 쪼개져서 해석되어 지고 실행될 것이다. 쪼개고 보니 `2+2`와 별차이가 없다.  `.` 은 프로퍼티 접근 연산자(Dot Operator, Property Accesor)이며 `console.log('Hello World')` 도 표현식이다.  함수 정의는 구문이지만 함수 호출은 표현식임을 아는 것이 중요하다. 자세한 내용은 [ch05 함수(Function)](/kickscar-javascript/basic-practices/tree/master/ch05) 를 참고한다.

​	마지막으로 그러면 눈 씻고 찾아보아도 `;` 와 구문이 1도 없는 앞의 예제 '값 4에 대한 다양한 표현식 : ex02.js' 의 모든 표현식은 실행되는 것인가? 실행되지 않는 것인가? 이 것은 ex02.js 가 모듈 실행인지 아닌지에 따라 다르다. 이 내용도  [ch11 모듈(Module)](/kickscar-javascript/basic-practices/tree/master/ch11) 를 참고한다.
...
```
 ...
 1번 라인은 숫자 리터럴 표현식 구문이고 2번 라인은 식별자 표현식 구문이다. 1번 라인 구문의 실행은 문제가 없지만 2번 라인 구문의 실행 시점에서는 변수 이름  `x` 로 저장한 값이 없기 때문에  Reference Error 가 발생한다. 따라서, 변수를 구문에서 사용하기 전에는 반드시 변수를 선언 또는 정의해야 한다. JavaScript 에서는 `let`, `const`, `var` 키워드를 사용해 변수의 선언 또는 정의를 할 수 있다. 

​	추천하지 않지만 4번 라인 처럼 세 키워드를 사용하지 않고  대입 연산자 표현식만으로도 정의가 가능하다. 하지만 ES6 코딩에서는 피해야 하고 조만간 오류가 될 것으로 예상된다. 

​	3번 라인은 변수에 값을 할당하지 않고 식별자로 변수의 이름만 부여 하였기 때문에 변수의 선언이 된다. 4번 라인은 식별자로 변수의 이름을 부여하고 그 이름에 값을 대입 하였기 때문에 변수의 정의가 된다.

​	출력 결과를 보면  x 가 `undefined` 로 출력된다.  [ch03/02. 데이터 타입](/kickscar-javascript/basic-practices/tree/master/ch03/02) 에서 다루는 내용이지만 `undefined`는 자바스크립트 기본 타입 중에 하나로 값을 명시적(explicit)으로 대입하지 않았지만 `undefined` 가 암시적(implicit)으로 대입되었다. 아래 코드처럼 명시적 대입도 가능하지만 굳이 하지 않는다. 따라서 JavaScript에서는 변수의 선언과 변수의 정의를 구분하지 않는다. 즉, 3번 라인도 정의로 보는 것이 JavaScript에서는 맞다.
...
​	간단히 정리하면, `let`과 `const`를 사용해서 변수를 정의하라는 것이고 자세한 그 이유는 [ch05 스코프(Scopre)](/kickscar-javascript/basic-practices/tree/master/ch05) 참고한다. 두 키워드 중  `let`은 변수 정의에 사용한다. `const`는 정의한 후 값이 변하지 않는 상수 정의에 사용한다. 상수는 한 번만 정의를 할 수 있는 것이 변수와의 차이점이다. 다음 예제를 보면 쉽게 이해할 수 있다.
